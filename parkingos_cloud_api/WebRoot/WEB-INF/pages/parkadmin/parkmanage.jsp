...
if(parseInt(role)==15||parseInt(role)==3){
	...
	var _localData = {
			"root_sys":{"id":"sys","name":"系统管理",icon:"home"}
			};
	for(var i=0;i<authmenu.length;i++){
		_localData["sys_"+i]=	{"id":i, "name":authmenu[i].nname};
		var url = 	authmenu[i].url+"?authid="+authmenu[i].auth_id;
		if(authmenu[i].nname=='参数设置')
			url +="&type=set"
		if(authmenu[i].url=='cartype.do')
			url +="&comid=${comid}"
		_localData["sys_"+i].fn=getFun(url);
	}
	//_localData["sys_1"]=	{"id":1, "name":"账号管理", fn:function(){GotoUrl('parkinfo.do')}};	
	//_localData["sys_2"]=	{"id":2, "name":"参数设置", fn:function(){GotoUrl('parkinfo.do?type=set')}};	
	//_localData["sys_5"]=	{"id":5, "name":"车型管理", fn:function(){GotoUrl('cartype.do?comid=${comid}')}};	
	//_localData["sys_3"]=	{"id":3, "name":"时租价格管理", fn:function(){GotoUrl('price.do') }};	
	//_localData["sys_4"]=	{"id":4, "name":"月卡套餐管理", fn:function(){GotoUrl('package.do')}};	
