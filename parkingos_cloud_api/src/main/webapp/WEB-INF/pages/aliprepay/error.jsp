	...
	<section>
		<div class="success">
			<div class="tips">
				...
				<div class="tips-title" id="noticetitle">红包找零成功</div>
				<!--<div class="tips-info prepay ">${parkname}</div> -->
				<!--<div class="tips-info epay "><span style="font-weight:bold;font-size:12px;">￥</span><span style="font-weight:bold;font-size:18px;">${change}</span></div>-->
			</div>
		</div>
...
<script type="text/javascript">
	...
	var type = '${state}';
    if(type=='0'){
        document.getElementById("image").setAttribute("class","icon-area error-icon");
        document.getElementById("noticetitle").innerHTML="<font color='red'>优惠券已经不能领取，请联系优惠券管理员</font>";
    }
    if(type=='1'){
        document.getElementById("image").setAttribute("class","icon-area error-icon");
        document.getElementById("noticetitle").innerHTML="<font color='red'>优惠券额度已经用完，请联系优惠券管理员</font>";
    }
    if(type=='2'){
        document.getElementById("image").setAttribute("class","icon-area error-icon");
        document.getElementById("noticetitle").innerHTML="<font color='red'>发生异常,红包领取失败</font>";
    }
    if(type=='3'){
        document.getElementById("image").setAttribute("class","icon-area error-icon");
        document.getElementById("noticetitle").innerHTML="<font color='red'>红包已被领取</font>";
    }
    if(type=='4'){
        document.getElementById("image").setAttribute("class","icon-area error-icon");
        document.getElementById("noticetitle").innerHTML="<font color='red'>车场余额不足,请联系车场负责人</font>";
    }

    if(type=='5'){
        document.getElementById("image").setAttribute("class","icon-area error-icon");
        document.getElementById("noticetitle").innerHTML="<font color='red'>账户余额不足,联系管理员</font>";
    }
