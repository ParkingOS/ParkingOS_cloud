...
public class ShopTicketManagerAction {
    ...
    public void shopTicket(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("create")) {
            Map<String, Object> rMap = createTicket(request, response, shop_id,uin);
...
//            }
            ...
            if(flag){//先判断车场是不是可以用券
                ...
                rMap.put("result", -1);
                rMap.put("error", "用券失败，本车场最多叠加"+superimposed+"张优惠券");
                AjaxUtil.ajaxOutput(response, rMap);
                return;
            }

            //判断商户自己的用券限制
            Integer useLimit = shopInfo.getUseLimit();
            logger.info("商户用券限制:"+useLimit);
            Integer shopCount = aliPrepayService.shopTicketCount(unpayOrder.getId(),shop_id);
            logger.info("商户已经用券shopCount:"+shopCount);
            if(useLimit>0){//如果useLimit==0，证明商户的设置和车场一样，车场都能用，商户肯定能用
                if(useLimit<=shopCount){
                    rMap.put("result", -1);
                    rMap.put("error", "用券失败，本商铺最多叠加"+useLimit+"张优惠券");
                    AjaxUtil.ajaxOutput(response, rMap);
                    return;
                }
            }
            ...
            //3 生成劵
            Map<String,Object> retMap = createTicket(request,response,shop_id,uin);
            ...
            retMap = noScanUseTicket(request, response, qrCodeTb.getTicketid(), unpayOrder.getCarNumber(), qrCodeTb.getComid(), unpayOrder);
            AjaxUtil.ajaxOutput(response, retMap);
        }
        return;
    }
     ...
     * @return
     */
    private Map<String, Object> createTicket(HttpServletRequest request, HttpServletResponse response, Long shop_id,Long uin) {
