...
public class StringUtils {
	...
	public static String createLinkedJson(Map<String, String> info,int type) {
		...
		if (info != null && info.size() > 0) {
			...
			for (int i = 0; i < keys.size(); i++) {
				...
				if (value == null || "".equals(value.trim()))
					continue;
				if(type==1)
					prestr += key + "=" + AjaxUtil.encodeUTF8(value+"") + "&";
				else {
					prestr += key + "=" + value+ "&";
				}
			}
	...
	public static void main(String[] args) {
		...
		System.out.println(AjaxUtil.encodeUTF8(AjaxUtil.encodeUTF8("京A88098")));//5天 21小时15分钟
		// 0.007232
			...
			for (Map<String, Object> map : info) {
				...
				for (String key : map.keySet()) {
					...
					if (v != null)
						v = v.toString().trim();
					json += "\"" + key + "\":\""
							+ StringUtils.encodeUTF8(v.toString()) + "\"";
					j++;
				}
				json += "}";
				i++;
