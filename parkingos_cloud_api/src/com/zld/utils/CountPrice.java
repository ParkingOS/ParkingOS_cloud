...
public class CountPrice {
	 ...
	 * @param end 结束utc时间
	 * @param dayMap 时段计费1
	 * @param nightMap 时段计费2 //分段计费时必须有，没有时，计费1变为全天的
	 * @return
	 */
		...
		if(dayMap!=null){//辅助价格只支持白天价格
			if(assistPrice!=null){
				...
				if(nightMap==null){
					...
					long et = 0L;
					try {
						bt = sdf.parse(sdf.format(date)).getTime()/1000+b_time*60*60;//进场时间当天的白天价格的起始点
						et = sdf.parse(sdf.format(date)).getTime()/1000+e_time*60*60;//进场时间当天的白天价格的结束点
					} catch (ParseException e) {
						e.printStackTrace();
					}
					if(start>=bt&&start<et){//进场时间在白天月卡时段内
						if(dfpt==1)//超过免费时长免费
							unit = unit+dft;
						if(dur>dft&&dur<=unit){//停车时长大于免费时长并且停车时长小于辅助时长
							if(start+unit*60>et){//跨时段了
...
//		start = calendar.getTimeInMillis()/1000;
		int sh = calendar.get(Calendar.HOUR_OF_DAY);//订单开始小时
...
//		end = calendar.getTimeInMillis()/1000;
		int eh = calendar.get(Calendar.HOUR_OF_DAY);//订单结束小时
		...
		if(nightMap==null){//没有夜间价格时，如果进场时间在夜间，则从日间计价时间开始计时
			if(start<ts){
				...
				sm = calendar.get(Calendar.MINUTE);//订单开始分钟
				if(end<start){
					end= start;
					eh = sh;
				}
		...
		if(sh<btime||(sh==btime&&sm<bminute))//价格时间 7-21，订单开始小时<7
			btype=0;//开始在夜间第一段
		...
		if(eh<btime||(eh==btime&&em<bminute))//价格时间 7-21，结束开始小时<7
			etype=0;//结束在夜间第一段
...
//				end = end -unit;
	 ...
	 * @param end 停车结束时间
	 * @param unit 价格时间单位
	 * @param priceunit1 第一小时单价
	 * @param priceunit2 第二小时单价
	 * @param priceunitoth 之后时间单价
	 * @param btime 开始计费小时
	 * @param etime 结束计费小时
	 * @param bmin 开始计费分钟
	 * @param emin 结束计费分钟
	...
	private static Double getAccountGuiLin(Long start, Long end, int unit,double priceunit1,double priceunit2,double priceunitoth,
		...
		if(end<start)
			return 0.0;
		if(type>1&&end-start<=600)//二三类区域10分钟内免费，超过10分钟，10分钟一并计入收费
			return 0.0;
		long countless = 0;//
		if(type==1)
			countless=60;//抹零1分钟
	...
	/**
	 * 是否是那种特殊车场 ，走特殊  （20160719目前不支持那边价格）
	 * @param comId 车场
	 * @return special 特殊车场列表
	 */
	...
	/**
	 * 台州计价分支1：
	 *:30分钟内免费，超过30分钟后前30分钟计费，首小时内5元，后每1小时3元，满整小时后的15分钟内不收费，24点封顶30元
	 * @param start
	 * @param end
	 * @param dayMap
	 * @param assistPrice
...
//		int car_type = Integer.parseInt(dayMap.get("car_type")+"");
		...
		calendar.set(Calendar.MINUTE, 0);
		calendar.set(Calendar.SECOND, 0);
		Long ts = calendar.getTimeInMillis()/1000;//日间开始时间的utc时间
		
		Long comid = (Long)dayMap.get("comid");
		//价格当日结束时间
		calendar.set(Calendar.HOUR_OF_DAY, etime);
		Long tte = calendar.getTimeInMillis()/1000;//日间结束时间的utc时间
		if(isspecialcar==1||(comid==20420||comid==20426)){
			calendar.set(Calendar.HOUR_OF_DAY, 17);
			calendar.set(Calendar.MINUTE, 30);

		}
		Long te = calendar.getTimeInMillis()/1000;//日间结束时间的utc时间
		Double price = assPrice;//总价格
		Long dur = end-start;
		if(start<ts)
			start = ts;//开始时间比价格的日间开始时间的utc时间小时，取价格日间开始时间的utc时间
		...
		for(int i=0;i<=days;i++){
			...
			if(end<=te){
				...
				if(isspecialcar == 1) {//特殊车辆
					//横街东路  横街西路 青年东路  青年西路，加免费30分钟
					if(comid==20429||comid==20422||comid==20427||comid==20421){
						if(start>=te){//特殊车辆在17:30后前入场 ，要免费半小时
							if(dur>30&&start<=tte){//时长大于30分钟且在结束时间之前入场，收费3元
								_price+=3;
							}
						}else {
	...
	public static Double getAccountTaizhou1(Long start,Long end,Map dayMap){
		...
		Integer countless=(Integer)dayMap.get("countless");//零头计费时长
		Long dur = (end-start)/60;
		if(dft>dur)//小于免费时长，直接返回
			return price;
		else if(dfpt==1)//超过免费时长免费时，扣除免费时长 
			dur = dur-dft;
		if(dftime>0){//日间首优惠时长
