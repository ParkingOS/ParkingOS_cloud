...
public class WeixinPrePayNewAction extends Action {
	
	private Logger logger = Logger.getLogger(WeixinPrePayNewAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		///http://192.168.0.188/zld/wxpreneworder.do?action=preorder&body=test&total_fee=100&attach=15801482643_1_1022_3_20140815
		...
		if (!"".equals(weixinToken)) {
			...
			packageParams.put("appid", Constants.WXPAY_APPID);
			packageParams.put("mch_id",  Constants.WXPAY_PARTNERID);//商户号
...
//			signParams.put("sign_method", "sha1");
			...
			//获取prepayId
			String prepayid = reqHandler.sendPrepay(packageValue,weixinToken);
			if (null != prepayid && !"".equals(prepayid)) {//成功取回，把数据返回给客户端，由客户端发请求完成支付。
				SortedMap<String, String> pMap = new TreeMap<String, String>();
				pMap.put("appid", Constants.WXPAY_APPID);
				//pMap.put("appkey", Constants.WXPAY_APPKEY);
				pMap.put("noncestr", noncestr);
				pMap.put("package", "Sign=WXPay");
				pMap.put("partnerid",Constants.WXPAY_PARTNERID);
				pMap.put("prepayid", prepayid);
...
//		 		_signParams.add(new NameValuePair("timestamp", timeStamp));
		 		String _sign =reqHandler._createSign(pMap);// genSign(_signParams);
