...
public class InspectGroupManageAction extends Action {
	@Autowired
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if(action.equals("")){
			...
			String sql = "select  a.id ,b.berthsec_name ,a.work_group_id,a.state  from   work_berthsec_tb as a  left join  com_berthsecs_tb as b on a.berthsec_id=b.id  where a.inspect_group_id=? and a.is_delete=?  ";
			String countSql = "select count(*)from work_berthsec_tb as a  left join  com_berthsecs_tb as b on a.berthsec_id=b.id where a.inspect_group_id=? and a.is_delete=? ";
			...
			String sql = "select  a.id ,b.nickname ,a.work_group_id,b.auth_flag  from   work_employee_tb  as a  left join  user_info_tb  as b on a.employee_id=b.id  where a.work_group_id=? and a.state=? and b.auth_flag =? ";
			...
			String sql = "select  a.id ,b.nickname ,a.inspect_group_id,b.auth_flag  from   work_inspector_tb  as a  left join  user_info_tb  as b on a.inspector_id=b.id  where a.inspect_group_id=? and a.state=? and b.auth_flag =? ";
			String countSql = "select count(*)from  work_inspector_tb  as a  left join  user_info_tb  as b on a.inspector_id=b.id  where a.inspect_group_id=? and a.state=? and b.auth_flag =? ";
			String fieldsstr = RequestUtil.processParams(request, "fieldsstr");
			List list = null;
			Long count = 0L;
			Integer pageNum = RequestUtil.getInteger(request, "page", 1);
			...
			String sql = "select  * from  inspect_group_tb  where  company_id=? " ;
			String countSql = "select count(*) from inspect_group_tb where company_id=? " ;
			String fieldsstr = RequestUtil.processParams(request, "fieldsstr");
			Integer pageNum = RequestUtil.getInteger(request, "page", 1);
			Integer pageSize = RequestUtil.getInteger(request, "rp", 20);
			List list = null;
			...
			String sql = "select * from com_berthsecs_tb  where id not in(   select berthsec_id from  work_berthsec_tb where is_delete=? and inspect_group_id>?) and is_active=? " ;
			String countSql = "select count(*) from com_berthsecs_tb  where id not in(   select berthsec_id from  work_berthsec_tb where is_delete=? and inspect_group_id>?) and  is_active=?  " ;
			String fieldsstr = RequestUtil.processParams(request, "fieldsstr");
			Integer pageNum = RequestUtil.getInteger(request, "page", 1);
			Integer pageSize = RequestUtil.getInteger(request, "rp", 20);
			List list = null;
			...
			daService.update("delete from  work_berthsec_tb  where inspect_group_id=?",
					new Object[]{id});
			daService.update("delete from work_inspector_tb  where inspect_group_id=? ",
					new Object[]{id});
			int r = daService.update("delete from inspect_group_tb where id=? ",
					new Object[]{id});
			AjaxUtil.ajaxOutput(response, "" + r);
		}
			...
			Long id = RequestUtil.getLong(request, "id", -1L);
			Map inspectotMap = daService.getMap("select inspector_id from work_inspector_tb  where id=? ",
					new Object[]{id});
	...
	private int editGroup(HttpServletRequest request){
		...
		int r = daService.update("update inspect_group_tb set inspectgroup_name=?,update_time=?,is_active=? where id=? ",
	...
	private int createBerthsec(HttpServletRequest request){
		...
		for(String id : cids){
			...
			r=daService.update("insert into work_berthsec_tb(inspect_group_id,berthsec_id,state) values(?,?,?)",
		...
		//int r=1;
		/*int r = daService.update("insert into inspect_group_tb(workgroup_name,create_time,is_active,company_id) values(?,?,?,?)", 
	...
	private int createInspector(HttpServletRequest request){
		...
		for(String id : cids){
			long ID = Long.parseLong(id);
			r=daService.update("insert into work_inspector_tb(inspect_group_id,inspector_id,state) values(?,?,?)",
					new Object[]{work_group_id,ID,0});
			//paramssimp +=",?";
			//params.add(Long.valueOf(id));
		}
