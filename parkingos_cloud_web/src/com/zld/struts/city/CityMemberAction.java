...
public class CityMemberAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if(action.equals("")){
			...
			String sql = "select u.* from user_info_tb u,user_role_tb r where u.role_id=r.id and u.cityid=? and u.state=? and r.state=? and r.oid=? ";
			String countSql = "select count(u.*) from user_info_tb u,user_role_tb r where u.role_id=r.id and u.cityid=? and u.state=? and r.state=? and r.oid=? ";
			...
			if(supperadmin == 1){
				...
				String sql = "select id as value_no,role_name as value_name from user_role_tb where state=? and oid=? and id<>? and adminid in (select id from user_info_tb where cityid=(select cityid from user_info_tb where id=? )) ";
			...
			int r = createMember(request, cityid, uin);
