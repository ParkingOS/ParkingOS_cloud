...
<div id = "constructor" style="margin:0px;padding:0px;overflow:hidden;width:100%;height:100%;position:relative;float:left">
	...
	<div id="right" style="position:absolute;padding:0px;margin:0px;top:0px;right:0px;overflow:hidden;border-top: 1px solid #ccc">
		...
		<div id="rightbot" style="width: 100%; margin:0px;padding:0px;border-top: 0px solid #ccc; position: relative; float: left">
			<iframe id="framePage0" name="framePage0" frameborder="0" scrolling="auto" style='width:100%;margin:0px;padding:0px;display:none'></iframe>
			<iframe id="framePage1" name="framePage1" frameborder="0" scrolling="auto" style='width:100%;margin:0px;padding:0px;display:none'></iframe>
		</div>
	</div>
</div>
...
if(parseInt(role)==15||parseInt(role)==3){
	...
	var frameO0 = T("#framePage0");
	var frameO1 = T("#framePage1");
	...
	frameO0.style.height = T.gwh() - t_h - r_t_h + "px";
	frameO1.style.height = T.gwh() - t_h - r_t_h + "px";
...
var authmenu = eval(T.A.sendData("getdata.do?action=getauthmenu&authid=${authid}"));
	var role=${role};
function getFun(url,index){
	return function(){GotoUrl(url,index)};
}
if(parseInt(role)==15||parseInt(role)==3){
	window.onload = jslimit()
}
	var _localData = {
			"root_sys":{"id":"sys","name":"${menuname}",icon:"home"}
			};

	for(var i=0;i<authmenu.length;i++){
		_localData["sys_"+i]=	{"id":i, "name":authmenu[i].nname};
		var subauths = authmenu[i].subauth;
		if(subauths&&subauths.length>0){
			for(var j=0;j<subauths.length;j++){
				_localData[i+"_"+i+"0"+j]=	{"id":i+"0"+j, "name":subauths[j].nname};
				var murl = authmenu[i].url;
				var surl = subauths[j].url+"?authid="+subauths[j].auth_id;
				if(murl.indexOf('?')!=-1)
					surl = subauths[j].url+"&authid="+subauths[j].auth_id;
				_localData[i+"_"+i+"0"+j].fn=getFun(surl,i);
			}
		}else{
			var murl = authmenu[i].url;
			...
			_localData["sys_"+i].fn=getFun(url,i);
		}
	}
	/**_localData["sys_1"]=	{"id":1, "name":"组织机构类型", fn:function(){GotoUrl('orgtype.do')}};	
	_localData["sys_2"]=	{"id":2, "name":"组织机构管理"};	
	...
	_localData["sys_5"]=	{"id":4, "name":"员工 管理", fn:function(){GotoUrl('authmanage.do')}};	
...
function GotoUrl(url,index){
	if(index==0){
		document.getElementById("framePage0").style.display='';
		document.getElementById("framePage1").style.display='none';
	}else{
		document.getElementById("framePage1").style.display='';
		document.getElementById("framePage0").style.display='none';
	}
	var obj = document.getElementById("framePage"+index);
	if(obj.src==''){
		obj.setAttribute('src',url);
	}
	//document.getElementById("framePage0").style.display='';
	//document.getElementById("framePage0").setAttribute('src',url);
}
</script>
<script>	
//系统菜单滚动
