...
function dataTypeSelect(name){
	...
	Twin({Id:"export_win",Title:"删除工单模板",Width:350,Height:h,sysfunI:sysfunI,sysfun:function(ids,tObj){
			Tform({
				formname: "opconfirm",
				formObj:tObj,
				dbuttons:false,
...
function CclientM(){//创建主操作菜单
	var bHtml = "";
	var auth_cc ="";
	bHtml += "<div style=\"margin-top:10px;margin-left:28%;width:225px;overflow:hidden;display:none\">";
	//bHtml+="<b><font size='4'>工单模板</font></b>";
	
	bHtml += "<div onclick=\"CreateC('taleview.do?action=add','创建工单模板')\" onmouseover=\"T('#add_tableview_bt').className='button_lh';T('#add_tableview_btr').className='button_rh'\" onmouseout=\"T('#add_tableview_bt').className='button_l';T('#add_tableview_btr').className='button_r'\"><div class=\"button_l\" style=\"width:88px\" "+auth_cc+" title=\"新建工单模板\" id=\"add_tableview_bt\"><span class=\"icon_add_c\" style=\"padding-left:18px\">新建工单模板</span></div><div class=\"button_r\" id=\"add_tableview_btr\" style=\"width:5px\"></div></div>";
	bHtml += "</div>";
...
function CclientS(){//创建搜索
	...
	mHtml += "<div style=\"margin-left:5px;height:21px;padding:0px;overflow:hidden;width:100%;white-space:nowrap\" title=\"不选择此项时,将搜索您的权限范围内的所有工单,包含客户池与回收站。\" >";
	//mHtml += " 工单模板";
	mHtml += "<div onclick=\"CreateBillModel('taleview.do?action=add')\" style='margin:5 auto' onmouseover=\"T('#add_tableview_bt').className='button_lh';T('#add_tableview_btr').className='button_rh'\" onmouseout=\"T('#add_tableview_bt').className='button_l';T('#add_tableview_btr').className='button_r'\"><div class=\"button_l\" style=\"width:88px\" title=\"新建工单模板\" id=\"add_tableview_bt\"><span class=\"icon_add_c\" style=\"padding-left:18px\">新建工单模板</span></div><div class=\"button_r\" id=\"add_tableview_btr\" style=\"width:5px\"></div></div>";
	mHtml += "</div>";
	mHtml +="</div>";
	mHtml += "</div>";
	...
	//mHtml += "<span style=\"width:100%;line-height:18px;float:left;text-align:center;font-weight:bold\"  onclick=\"supperSerach(getCleintFields(1),null,getVisitor_region,initArea)\" title=\"高级搜索\">高级搜索</span>";
	//mHtml += "<span style=\"width:49%;line-height:22px;float:left;text-align:center;border-left:1px solid #D0D0D0\" title=\"组合搜索\">组合搜索</span>";
	mHtml += "</div>";
	mHtml += "</form>";
	...
	var ssHtml = "";
		...
		//ssHtml += "    <div class=\'t_items_out\' tid=\"cphone\">固定电话<\/div>";
		ssHtml += "  <\/div>";
...
function SimpleSearch(title){
	...
	//T("#ss_type_range").checked != true?title = "所有会话":"";
...
function CclientD(){//创建数据分类
	layoutL_B_T.innerHTML = "<span style=\"padding-left:3px;float:left;\">工单模板分类</span>";
	var Cspan = document.createElement("span");
	Cspan.className = "button";
	Cspan.style.Float = "right";
	Cspan.title = "更新分类数据总数";
	Cspan.innerHTML = "<img src=\"images/form/reload.gif\" style=\"vertical-align:middle;border:0px;\" \/><font style=\"font-weight:normal\">刷新</font>";
...
function CreateBillModel(url){
	if(tud_id==''){
		T.loadTip(1,"请先创建工单表自定义字段（系统自定义->工单表自定义->工单表自定义字段）！",10,"");
		return;
	}
