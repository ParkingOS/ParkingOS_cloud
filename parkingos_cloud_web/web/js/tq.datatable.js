...
//			if(data==null||typeof(data)!="object"){T.loadTip(1,"数据格式错误！",2,"");return};
			if(data&&typeof(data)=="object"&&data.total&&parseInt(data.total)>0){
				...
				if(data.summary){
					if(!T("#summary_money")){
						var paatool = document.getElementById(TNM+"_title_div");
						paatool.innerHTML += "&nbsp;&nbsp;<font id='summary_money' style='font-size:12px;color:red'>合计："+data.summary+"</font>";
					}
				}
				...
				for (var i=0;i<tds.length;i++)
				{
					...
					//普通列
					tablecontent.push("<tr id=\""+TNM+"_"+tds[i].id+"_tr\" rid=\""+tds[i].id+"\"  onclick='table_chanage_color(this)' class=\"t-grid-row\" \"");
...
function table_chanage_color(obj){
	//alert(obj.className);
	$(obj).parent().find('tr').removeClass('t-grid-row_sel');
	obj.className='t-grid-row_sel';
};