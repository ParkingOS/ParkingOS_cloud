...
if(parseInt(role)==15||parseInt(role)==3){
	...
	for(var i=0;i<authmenu.length;i++){
		...
		var url = "";
		if(authmenu[i].url.indexOf("?") > -1){
			url = authmenu[i].url+"&authid="+authmenu[i].auth_id;
		}else{
			url = authmenu[i].url+"?authid="+authmenu[i].auth_id;
		}
	...
	//_localData["sys_1"]=	{"id":1, "name":"角色管理", fn:function(){GotoUrl('adminrole.do')}};	
	//_localData["sys_2"]=	{"id":2, "name":"员工管理", fn:function(){GotoUrl('member.do')}};	
