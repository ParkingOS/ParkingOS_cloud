...
<link href="css/iconbuttons.css" rel="stylesheet" type="text/css">
	...
	<script type="text/javascript" src="js/tq.mulSelect.js"></script>
...
<script type="text/javascript">
	var tree = eval(T.A.sendData("${treeurl}"));
	var selnode = "0";//出始被选中间那个节点
	function getFun(url){
		return function(){GotoUrl(url)};
	}
	var _localData = {};
	...
	for(var i=0;i<tree.length;i++){
		...
		var sysid = tree[i].sysid;
		var treeid = tree[i].treeid;
		var url = tree[i].url;
		if(treeid == 0){
			selnode = tree[i].selnode;
			_localData[sysid]=	{"id":treeid, "name":name,icon:"home"};
		}else{
			_localData[sysid]=	{"id":treeid, "name":name,checked: "1"};
		}
		_localData[sysid].fn=getFun(url);
		/* _localData["sys_"+i]=	{"id":i, "name":tree[i].name};
		...
		if(subauths&&subauths.length>0){
			for(var j=0;j<subauths.length;j++){
				...
				_localData[i+"_"+i+"0"+j].fn=getFun(surl);
			}
		}else{
			var murl = authmenu[i].url;
			...
			_localData["sys_"+i].fn=getFun(url);
		} */
	}
	...
	sysMenuTree = new tqTree({
		...
		loadfun:function(v){
			var initID = sysMenuTree.nodes[selnode].childNodes["0"].id;
			sysMenuTree.focusNode(initID);//定位到节点
			sysMenuTree.expandLevel(3);
			var nodes = sysMenuTree.nodes;
		...
		checkFun : function(node){
			sysMenuTree.getChecked();
			sysMenuTree.nodes;
