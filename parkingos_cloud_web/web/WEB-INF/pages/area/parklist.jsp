...
var areas = T.A.sendData("getdata.do?action=getareas&groupid=${groupid}");
var _mediaField = [
		...
		{fieldcnname:"所属区域",fieldname:"areaid",fieldvalue:'',defaultValue:'',inputtype:"select",noList:areas, twidth:"60" ,height:"",issort:false}
		];
var rules =[
	...
	bus.push({dname:"注册停车场",icon:"edit_add.png",onpress:function(Obj){
		...
		Twin({Id:"parking_add",Title:"添加停车场",Width:550,sysfun:function(tObj){
				Tform({
					...
					formObj:tObj,
					recordid:"id",
					suburl:"orgpark.do?action=create",
					method:"POST",
					formAttr:[{
...
function getAuthIsoperateButtons(){
	...
	bts.push({name:"编辑",fun:function(id){
		...
		Twin({Id:"parking_edit_"+id,Title:"编辑",Width:550,sysfunI:id,sysfun:function(id,tObj){
				Tform({
					...
					formObj:tObj,
					recordid:"parking_id",
					suburl:"orgpark.do?action=edit&id="+id,
					method:"POST",
					formAttr:[{
	...
	bts.push({name:"设置",fun:function(id){
		Twin({
			...
			Content:"<iframe src=\"orgpark.do?action=set&id="+id+"\" style=\"width:100%;height:100%\" frameborder=\"0\"></iframe>",
			Width:T.gww()-100,
			Height:T.gwh()-50
		})
		}});
