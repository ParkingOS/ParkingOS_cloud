...
var _field = [
		...
		{fieldcnname:"道闸机名称",fieldname:"brake_name",defaultValue:'',fieldvalue:'',inputtype:"text", twidth:"160" ,height:"",issort:false},
		{fieldcnname:"串口号",fieldname:"serial",fieldvalue:'',inputtype:"text",twidth:"100" ,height:"",issort:false},
		{fieldcnname:"设备地址IP",fieldname:"ip",fieldvalue:'',defaultValue:'554',inputtype:"text",twidth:"100" ,height:"",issort:false},
...
var rules =[{name:"brake_name",requir:true},{name:"passid",requir:true}];
var _cameraT = new TQTable({
	tabletitle:"道闸机管理",
	ischeck:false,
	tablename:"camera_tables",
	dataUrl:"citybrake.do",
	iscookcol:false,
	//dbuttons:false,
...
function getAuthButtons(){
	var bts=[];
	if(subauth[1])
		bts.push({dname:"添加道闸机",icon:"edit_add.png",onpress:function(Obj){
		T.each(_cameraT.tc.tableitems,function(o,j){
			o.fieldvalue ="";
		});
		Twin({Id:"camera_add",Title:"添加道闸机",Width:550,sysfun:function(tObj){
				Tform({
					formname: "camera_edit_f",
					formObj:tObj,
					recordid:"id",
					suburl:"citybrake.do?action=create",
					method:"POST",
					Coltype:2,
					formAttr:[{
	...
	bts.push({dname:"高级查询",icon:"edit_add.png",onpress:function(Obj){
		...
		Twin({Id:"camera_search_w",Title:"搜索停车场",Width:550,sysfun:function(tObj){
				TSform ({
...
function getAuthIsoperateButtons(){
	...
	bts.push({name:"编辑",fun:function(id){
		...
		Twin({Id:"camera_edit_"+id,Title:"编辑",Width:550,sysfunI:id,sysfun:function(id,tObj){
				Tform({
					...
					formObj:tObj,
					recordid:"camera_id",
					suburl:"citybrake.do?action=edit&id="+id,
					method:"POST",
					Coltype:2,
					formAttr:[{
	...
	bts.push({name:"删除",fun:function(id){
		var id_this = id ;
		Tconfirm({Title:"确认删除吗",Content:"确认删除吗",OKFn:function(){T.A.sendData("citybrake.do?action=delete","post","selids="+id_this,
