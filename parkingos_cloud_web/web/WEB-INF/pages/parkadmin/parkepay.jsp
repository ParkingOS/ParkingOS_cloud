...
if(parseInt(role)==15||parseInt(role)==3){
	...
	var _localData = {
			"root_sys":{"id":"sys","name":"电子支付",icon:"home"}
			};
	for(var i=0;i<authmenu.length;i++){
		...
		if(authmenu[i].url=='parkaccountaly.do')
			url +="&isparkcloud=1"
		_localData["sys_"+i].fn=getFun(url);
	}
	///_localData["sys_1"]=	{"id":1, "name":"提现管理", fn:function(){GotoUrl('withdraw.do')}};	
	//_localData["sys_2"]=	{"id":2, "name":"电子收款", fn:function(){GotoUrl('parkaccountaly.do?isparkcloud=1') }};	
