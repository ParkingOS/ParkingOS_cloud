...
@interface TPostCommentViewController ()<UITextViewDelegate, MBProgressHUDDelegate, UIScrollViewDelegate>
...
@implementation TPostCommentViewController
...
- (void)buttonTouched:(UIButton*)button {
    ...
    NSMutableDictionary* parmas = [NSMutableDictionary dictionaryWithDictionary:@{
                             @"action" : @"comment",
                             @"mobile" : [[NSUserDefaults standardUserDefaults] objectForKey:save_phone],
                             @"comment" : _commentTextView.text}];
    [parmas setObject:_parkId forKey:@"comid"];
    ...
    [self.model sendRequest:_sendRequest completion:^(NSDictionary *result, NSError *error) {
        ...
        if ([[result objectForKey:@"info"] isEqualToString:@"1"]) {
            ...
            [TAPIUtility alertMessage:@"对不起，评价失败～" success:NO toViewController:nil];
        }
    }];
}
