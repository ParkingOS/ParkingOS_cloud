...
public class HXNotifier {
    ...
    protected static int mNotifyID = 0x00001; // start notification id
    ...
    Map<String,Integer> historyNotices = new HashMap<>();
    UserDao mUserDao;
...
//    }
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
            ...
            if (historyNotices.containsKey(form)) {
                notifyID = historyNotices.get(form);
                ...
                summaryBody =String.format("[%d条]%s:%s", unread, name, ticker);
            } else {
                notifyID = ++mNotifyID;
                summaryBody =String.format("%s:%s", name, ticker);
                historyNotices.put(form, notifyID);
            }
            ...
            //多个好友Lai消息，显示好友列表
//            if (fromUsersNum > 1) {
//                msgIntent = notificationInfoProvider.getLaunchMoreMessageIntent();
//            }
...
//                notificationManager.cancel(notifyID);
    ...
    public interface HXNotificationInfoProvider {
        ...
        /**
         * 设置notification点击时，多个用户发来消息跳转intent
         * @return
         */
        Intent getLaunchMoreMessageIntent();
    }
}
