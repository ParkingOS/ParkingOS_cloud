...
public class HXSDKHelperImpl extends HXSDKHelper {
    ...
    private static final String TAG = "HXSDKHelperImpl";
        ...
        eventListener = new EMEventListener() {
            ...
            public void onEvent(EMNotifierEvent event) {
                ...
                if(event.getData() instanceof EMMessage){
                    ...
                    LogUtils.d("receive the event : " + event.getEvent() + ",id : " + message.getMsgId() + ":"+event.getEvent().name());
                }
    ...
    protected HXNotifier.HXNotificationInfoProvider getNotificationListener() {
        ...
        return new HXNotifier.HXNotificationInfoProvider() {
            ...
            public String getDisplayedText(EMMessage message) {
                ...
                User user = mUserDao.getContact(message.getFrom());
            ...
            public Intent getLaunchIntent(EMMessage message) {
                //设置点击通知栏跳转事件
                User friend = mUserDao.getContact(message.getFrom());
            ...
            public Intent getLaunchMoreMessageIntent() {
                Intent intent = new Intent(appContext, FriendActivity.class);
                return intent;
            }
        };
    }
        ...
        return new HXNotifier(){
            public synchronized void onNewMsg(final EMMessage message) {
                ...
                if (message.getChatType() == ChatType.Chat) {
                    ...
                    notNotifyIds = ((HXSDKModelImpl) hxModel).getDisabledGroups();
                } else {
                    chatUsename = message.getTo();
                    notNotifyIds = ((HXSDKModelImpl) hxModel).getDisabledIds();
                }

                if (notNotifyIds == null || !notNotifyIds.contains(chatUsename)) {
                    // 判断app是否在后台
                    LogUtils.i("判断app是否在后台" + EasyUtils.getTopActivityName(appContext));
                    if (!EasyUtils.isAppRunningForeground(appContext)) {
                        EMLog.d(TAG, "app is running in backgroud");
                        sendNotification(message, false);
                        viberateAndPlayTone(message);
                    } else {
                        String chatActivityString = ChatActivity.class.getName();
                        if (chatActivityString.equals(EasyUtils.getTopActivityName(appContext))) {
//                            sendNotification(message, true);
                        } else {
                            EMLog.d(TAG, "app is not running chatactivity");
                            sendNotification(message, false);
                            viberateAndPlayTone(message);
                        }
    ...
    public Map<String, User> getContactList() {
        if (getHXId() != null && contactList == null) {
            contactList = ((HXSDKModelImpl) getModel()).getContactList();
        }
    ...
    public void saveContact(User user){
    	...
    	((HXSDKModelImpl) getModel()).saveContact(user);
    }
...
//    }
    ...
    public void updateContactList(List<User> contactInfoList) {
        ...
        ((HXSDKModelImpl)getModel()).saveContactList(mList);
    }
