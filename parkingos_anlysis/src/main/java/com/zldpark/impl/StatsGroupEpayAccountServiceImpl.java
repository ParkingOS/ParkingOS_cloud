...
import com.zldpark.pojo.GroupEpayAccount;
	...
	public AccountResp account(AccountReq req) {
		...
		try {
			...
			String sql = "select * from park_account_tb where is_delete=? and create_time " +
					...
					" create_time desc ";
			List<GroupEpayAccount> groupEpayAccounts = readService.getPOJOList(sql, params, pageNum, pageSize,
					GroupEpayAccount.class);
			resp.setGroupEpayAccounts(groupEpayAccounts);
			resp.setResult(1);
			return resp;
		} catch (Exception e) {
			e.printStackTrace();
