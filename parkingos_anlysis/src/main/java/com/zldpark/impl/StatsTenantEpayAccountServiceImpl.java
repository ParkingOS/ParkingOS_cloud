...
import com.zldpark.pojo.TenantEpayAccount;
	...
	public AccountResp account(AccountReq req) {
		...
		try {
			...
			String sql = "select * from park_account_tb where is_delete=? and create_time " +
					...
					" create_time desc ";
			List<TenantEpayAccount> tenantEpayAccounts = readService.getPOJOList(sql, params, pageNum, pageSize,
					TenantEpayAccount.class);
			resp.setTenantEpayAccounts(tenantEpayAccounts);
			resp.setResult(1);
			return resp;
		} catch (Exception e) {
			e.printStackTrace();
