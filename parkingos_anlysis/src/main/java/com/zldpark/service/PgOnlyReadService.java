...
public class PgOnlyReadService {
	...
	public List<Map<String, Object>> getAll(String sql, Object[] values, int pageNum, int pageSize ){
		if(pageNum > 0 && pageSize > 0){
			...
			if(values == null){
				...
				values2 = new Object[values.length + 2];
				for(int i = 0; i < values.length; i++){
					values2[i] = values[i];
				}
			}
			values2[values2.length -2] = pageSize;
			values2[values2.length -1] = start;
			values = values2;
		}
		return userDao.getAll(sql, values);
	}
